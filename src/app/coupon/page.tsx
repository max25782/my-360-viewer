...
export default function CouponPage() {
  ...
  // Mock function for opening chat - replace with actual implementation
  const handleOpenChat = (message: string | any) => {
    console.log('Opening chat with message:', message);
    // Here you would typically open a chat modal or redirect to chat
    if (typeof message === 'string') {
      alert(`Chat opened with message: ${message}`);
    } else {
      alert(`Chat opened with offer: ${message.offer?.title || 'Unknown offer'}`);
    }
  };
      ...
      <div className="max-w-6xl mx-auto pt-20">
        ...
        >
          ...
          <span className="text-emerald-300 text-sm font-medium">
            Connected to: Michael Chen • 2847 Pine Street, Seattle, WA 98101
          </span>
          <Button
            size="sm"
            className="ml-2 bg-slate-700/50 hover:bg-slate-600/50 text-slate-300 border-slate-600/50"
          >
                  ...
                  <div className="p-4 pt-2 relative z-20">
                    <Button
                      onClick={() => {
                        ...
                        // Send the offer data object to chat
                        handleOpenChat({
                          offer: offer,
        ...
        {/* Bottom Note */}
        <div className={`text-center text-sm mt-8 p-4 rounded-xl ${
          isDark ? 'bg-slate-800/30 text-slate-400' : 'bg-slate-50/50 text-slate-500'
        }`}>
          💬 Click "Claim with Daniel" to start your conversation • Response within 2 hours • Free consultation
        </div>
      </div>
    </div>
  );
