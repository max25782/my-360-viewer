/**
 * Script for generating skyline comparison images manifest
 */
...
const ASSETS_DIR = path.join(__dirname, '../public/assets/skyline');
// Path to save manifest
const MANIFEST_PATH = path.join(__dirname, '../public/skyline-comparison-manifest.json');

// Function to scan directories
function scanDirectories() {
  console.log('🔍 Scanning skyline house directories for comparison images...');
  ...
  console.log(`📂 Found ${houses.length} skyline houses`);
  ...
  houses.forEach(house => {
    ...
    const files = fs.readdirSync(comparisonDir, { withFileTypes: true })
      ...
      .map(file => ({
        filename: file.name,
        path: `/assets/skyline/${house}/comparison/${file.name}`,
...
// Main function
function main() {
  console.log('🚀 Starting generation of skyline comparison images manifest...');
